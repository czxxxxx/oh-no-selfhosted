...
try {
  const packResult = run("npm", ["pack", "--json", "--silent", "--pack-destination", temporaryRoot]);
  const jsonStart = packResult.stdout.indexOf("[");
  if (jsonStart < 0) {
    throw new Error(`npm pack did not return its JSON manifest\n${packResult.stdout}`);
  }
  const [{ filename }] = JSON.parse(packResult.stdout.slice(jsonStart));
  const tarballPath = join(temporaryRoot, filename);
  const installRoot = join(temporaryRoot, "install");

  run("npm", ["install", "--prefix", installRoot, "--ignore-scripts", "--no-audit", "--no-fund", tarballPath]);

  const packageRoot = join(installRoot, "node_modules", "oh-no-selfhosted");
